...
if(isset($_POST['checkInHall']) && isset($_POST['checkOutHall']))
{
    //  echo "Error";
    
      $checkIn = $_POST['checkInHall'];
      $checkOut = $_POST['checkOutHall'];
    ...
    //  $sqld = "SELECT * FROM room WHERE NOT EXISTS (SELECT NULL FROM reservation WHERE room.room_Id = reservation.room_Id AND reservation.dateIn <= '$checkOut' AND '$checkIn' <= reservation.dateOut)";
     $sqld = "SELECT * FROM hall WHERE NOT EXISTS (SELECT NULL FROM reservation WHERE hall.hall_Id = reservation.hall_Id AND reservation.dateIn <= '$checkOut' AND '$checkIn' <= reservation.dateOut AND reservation.reservationStatus != 'Unpaid')";
