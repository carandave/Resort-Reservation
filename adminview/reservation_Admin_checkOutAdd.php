			...
			for($i = 0; $i<count($file_array); $i++){
				if($file_array[$i]['error']){
					...
					if(!in_array($file_ext, $extensions)){
						...
						$query = "UPDATE reservation SET reservationStatus='Confirmed', code='$code', downCheckOut='$out_downpayment', payment_status='$out_inititalPayment' WHERE reservation_Id='$out_reservationId'";
						$query_run = mysqli_query($conn, $query);
