...
<div class="row ml-5">
  ...
  <div class="card-body m-5" id="printThis">
    ...
    $sqlf = "SELECT SUM(downpayment) AS sum FROM payment WHERE date_of_pay BETWEEN '$from_date' AND '$to_date'";
    ...
    <?php while($rowf = mysqli_fetch_assoc($sqlf_result)){?>
        <h6 class="mt-3 text-light bg-danger text-center p-1" style="width: 600px;">Total Sales From <?php echo $from_date?> To <?php echo $to_date?> : <?php echo $rowf['sum'];?> Pesos      </h6>
    ...
    <?php } ?>
        ...
        <tbody>
            ...
            $sql = "SELECT p.payment_Id, p.people_Id, p.reservation_Id, p.total, p.downpayment, p.balance, p.reference, p.payment_status, p.date_of_pay, u.people_Id, u.fName, u.lName FROM payment p INNER JOIN people u ON p.people_Id = u.people_Id WHERE p.date_of_pay BETWEEN '$from_date' AND '$to_date' ORDER BY p.reservation_Id DESC ";
